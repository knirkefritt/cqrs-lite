...
abstract class AggregateRoot(var id: UUID = DefaultUUID) {
    companion object {
         ...
         */
        internal fun loadFromHistory(aggregateRoot: AggregateRoot, history: Iterable<Event>) {
            for (e in history) {
                val eventVersion = e.version
                val eventId = e.id ?: throw MissingEventIdException(e.javaClass)

                if (eventVersion != aggregateRoot.version + 1) {
                    throw EventsOutOfOrderException(e.id ?: DefaultUUID)
                }
                if (eventId != aggregateRoot.id && aggregateRoot.id != DefaultUUID) {
                    throw EventIdIncorrectException(eventId, aggregateRoot.id)
                }
                aggregateRoot.applyEvent(e)
                aggregateRoot.id = eventId
